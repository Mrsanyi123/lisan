...
import { Home, Compass, User as UserIcon, Zap, Heart, Gem, Globe, Star, MessageCircle, Lock } from 'lucide-react';
import { NovaMascot, Button, StatBadge, ProgressBar, VideoBackground } from './components/ui';
...
const LESSON_PATH: Lesson[] = [
  { id: '1', title: 'Basics 1', description: 'Greetings', locked: false, completed: true, stars: 3, color: 'green', position: 'center' },
  { id: '2', title: 'Basics 2', description: 'Common Phrases', locked: false, completed: true, stars: 2, color: 'green', position: 'left' },
  { id: '3', title: 'Food', description: 'Ordering & Eating', locked: false, completed: false, stars: 0, color: 'purple', position: 'right' },
  { id: '4', title: 'Animals', description: 'Farm & Wild', locked: true, completed: false, stars: 0, color: 'green', position: 'center' },
  { id: '5', title: 'Family', description: 'Members', locked: true, completed: false, stars: 0, color: 'green', position: 'left' },
  { id: '6', title: 'Travel', description: 'Directions', locked: true, completed: false, stars: 0, color: 'yellow', position: 'right' },
];
    ...
    return (
      <VideoBackground>
        <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
          ...
          <h1 className="text-5xl font-extrabold text-white mb-4 tracking-wide drop-shadow-lg">NOVA</h1>
          ...
          <div className="w-full max-w-sm space-y-4">
            <Button fullWidth variant="primary" onClick={() => setScreen(Screen.SIGNUP)} className="shadow-lg shadow-blue-500/30 border-none">
              GET STARTED
            </Button>
            <Button fullWidth variant="secondary" onClick={() => setScreen(Screen.LOGIN)} className="bg-white/10 border-white/20 text-white hover:bg-white/20 backdrop-blur-md">
  ...
  if (screen === Screen.LOGIN || screen === Screen.SIGNUP) {
    return (
      <AuthScreen 
        ...
        onSwitchMode={() => setScreen(screen === Screen.LOGIN ? Screen.SIGNUP : Screen.LOGIN)}
        onSuccess={() => {
        ...
        <div className="min-h-screen p-6">
          <div className="max-w-4xl mx-auto mt-10">
            <div className="flex items-center mb-8">
               <button onClick={() => setScreen(Screen.WELCOME)} className="mr-4 text-white/70 hover:text-white transition-colors bg-white/10 p-2 rounded-full backdrop-blur-md">
                 <Globe size={24}/>
               </button>
               <h2 className="text-3xl font-bold text-white drop-shadow-md">Choose your path</h2>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              {LANGUAGES.map(lang => (
                ...
                >
                  <span className="text-6xl filter drop-shadow-lg group-hover:scale-110 transition-transform">{lang.flag}</span>
                  <span className="font-bold text-white text-lg tracking-wide">{lang.name}</span>
  ...
  if (screen === Screen.LESSON) {
    return (
      <LessonScreen 
        ...
        onComplete={(xp) => {
          setStats(prev => ({ ...prev, xp: prev.xp + xp, gems: prev.gems + 10 }));
          setScreen(Screen.HOME);
        }}
      />
    );
    ...
    <div className="min-h-screen bg-nova-bg pb-24 md:pb-0 md:pl-64">
      ...
      <nav className="fixed bottom-0 left-0 w-full md:w-64 md:h-screen bg-white border-t md:border-t-0 md:border-r border-gray-200 z-50 flex md:flex-col justify-around md:justify-start md:p-4">
        <div className="hidden md:flex items-center gap-2 mb-8 px-4">
           ...
           <span className="text-2xl font-extrabold text-nova-primary tracking-wide">NOVA</span>
        </div>
        ...
        {isProfile && (
           <div className="space-y-6">
             <div className="flex flex-col items-center border-b pb-8">
               ...
               <h2 className="text-2xl font-bold text-gray-800">Language Explorer</h2>
             ...
             <div className="grid grid-cols-2 gap-4">
                <div className="border-2 border-gray-200 rounded-2xl p-4">
                   ...
                   <div className="text-2xl font-black text-nova-primary">{stats.xp}</div>
                </div>
                <div className="border-2 border-gray-200 rounded-2xl p-4">
                   <h3 className="font-bold text-gray-600 mb-2">Current Streak</h3>
                   <div className="text-2xl font-black text-orange-500">{stats.streak} Days</div>
                </div>
             </div>
             ...
             <div className="border-2 border-gray-200 rounded-2xl p-4">
                ...
                <div className="space-y-4">
                   <AchievementRow title="Wildfire" desc="Reach a 3 day streak" level={1} max={3} icon="🔥" color="bg-orange-500" />
                   <AchievementRow title="Sage" desc="Earn 1000 XP" level={3} max={3} icon="🎓" color="bg-nova-secondary" />
                   <AchievementRow title="Scholar" desc="Learn 50 new words" level={2} max={5} icon="📚" color="bg-blue-500" />
                </div>
             </div>
           </div>
        )}
...
const NavItem: React.FC<{ icon: any, label: string, isActive: boolean, onClick: () => void, badge?: string }> = ({ icon: Icon, label, isActive, onClick, badge }) => (
  <button 
    onClick={onClick}
    className={`flex md:flex-row flex-col items-center md:gap-4 p-2 md:px-4 md:py-3 rounded-xl transition-colors relative
      ${isActive ? 'bg-blue-50 text-nova-primary border-blue-200 md:border-2' : 'text-gray-400 hover:bg-gray-100'}
    `}
  >
    <Icon className="w-6 h-6 md:w-5 md:h-5" strokeWidth={isActive ? 2.5 : 2} />
    <span className={`text-[10px] md:text-sm font-bold uppercase tracking-wide mt-1 md:mt-0`}>{label}</span>
    {badge && (
...
const LessonNode: React.FC<{ lesson: Lesson, onClick: () => void }> = ({ lesson, onClick }) => {
  const getOffset = () => {
  ...
  const getColor = () => {
     ...
     if (lesson.color === 'purple') return 'bg-nova-secondary border-nova-secondaryDark'; // Black/Slate
     if (lesson.color === 'yellow') return 'bg-nova-accent border-nova-accentDark'; // Sky Blue
...
const AchievementRow: React.FC<{ title: string, desc: string, level: number, max: number, icon: string, color: string }> = ({ title, desc, level, max, icon, color }) => (
  <div className="flex items-center gap-4">
    <div className={`w-14 h-14 rounded-full ${level === max ? color : 'bg-gray-200'} flex items-center justify-center text-2xl text-white`}>
      {icon}
    </div>
    <div className="flex-1">
      <div className="flex justify-between items-center mb-1">
        <h4 className="font-bold text-gray-700">{title}</h4>
        <span className="text-sm font-bold text-gray-400">{level}/{max}</span>
      </div>
      <p className="text-xs text-gray-500 mb-2">{desc}</p>
      <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden">
        <div className={`h-full ${color}`} style={{ width: `${(level/max)*100}%` }}></div>
      </div>
    </div>
  </div>
);
